...
uint8_t NRF24L01_Check(void)
{
	...
	NRF24L01_Write_Buf(0x20+TX_ADDR,buf,5);//写入5个字节的地址.	
...
//  	for(uint8_t_ctr=0; uint8_t_ctr<len; uint8_t_ctr++)SPIx_ReadWriteByte(*pBuf++); //写入数据	 
		SPIx_ReadWriteByte(reg);
...
uint8_t NRF24L01_TxPacket(uint8_t *txbuf)
{
	...
	NRF24L01_Write_Reg(0x20+STATUS,sta); //清除TX_DS或MAX_RT中断标志
...
void NRF24L01_RX_Mode(void)
{
	uint8_t sta;
	NRF24L01_CE_L;	  
  	NRF24L01_Write_Buf(0x20+RX_ADDR_P0,(uint8_t*)RX_ADDRESS,RX_ADR_WIDTH);//写RX节点地址
	  
  	NRF24L01_Write_Reg(0x20+EN_AA,0x01);    //使能通道0的自动应答    
  	NRF24L01_Write_Reg(0x20+EN_RXADDR,0x01);//使能通道0的接收地址  	 
  	NRF24L01_Write_Reg(0x20+RF_CH,40);	     //设置RF通信频率		  
  	...
  	NRF24L01_Write_Reg(0x20+CONFIG, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
		...
		NRF24L01_Write_Reg(FLUSH_RX,0xff);//清除RX FIFO寄存器  n
...
{														 
		NRF24L01_CE_L;	    
  	NRF24L01_Write_Buf(0x20+TX_ADDR,(uint8_t*)TX_ADDRESS,TX_ADR_WIDTH);//写TX节点地址 
  	NRF24L01_Write_Buf(0x20+RX_ADDR_P0,(uint8_t*)RX_ADDRESS,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  

  	NRF24L01_Write_Reg(0x20+EN_AA,0x01);     //使能通道0的自动应答    
  	NRF24L01_Write_Reg(0x20+EN_RXADDR,0x01); //使能通道0的接收地址  
  	NRF24L01_Write_Reg(0x20+SETUP_RETR,0x1a);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
  	NRF24L01_Write_Reg(0x20+RF_CH,40);       //设置RF通道为40
  	NRF24L01_Write_Reg(0x20+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
  	NRF24L01_Write_Reg(0x20+CONFIG,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断
